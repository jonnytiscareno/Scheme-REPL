...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            target_formals, target_vals = formals.first, vals.first
            child.define(target_formals, target_vals)
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        arguments = operands.map(lambda operand: scheme_eval(operand, env))
...
class MacroProcedure(LambdaProcedure):
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN Problem 21
        exp = Thunk(operands, env)
        expression_mac = complete_eval(scheme_apply(self, exp, env))
        return expression_mac
        # END Problem 21
...
def do_or_form(expressions, env):
    ...
    if scheme_truep(x):
        return x
    elif expressions.second == nil and scheme_falsep(x) == False:
        return False
    else:
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    # Exact replication of do_define_form with 1 slight alteration
    check_form(expressions, 2)
    target = expressions.first
    if scheme_symbolp(target):
        check_form(expressions, 2, 2)
        ...
        val = MacroProcedure(target.second, expressions.second, env) # Just change this to be a MacroProcedure instead of LambdaProcedcure
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            # START EC 21 Adaption
            if isinstance(procedure, MacroProcedure): #EC #21
                mac_expr = complete_eval(scheme_apply(procedure, rest, env))
                result = scheme_eval(mac_expr, env)
            # END EC 21 Adaption
            else:
